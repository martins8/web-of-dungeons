...
function makeAttacker(
  { skills = [] as any[], currentHp = 100, maxHp = 100, stats = {} } = {},
  cdIds: string[] = [],
  cc = false,
) {
  ...
  return {
    ...
    health: { currentHp },
    combatState: {
      isOnCooldown: (skill: any) => cdIds.includes(skill.id),
...
describe("Normal AI - decideSkill", () => {
  ...
  test("selects support heal when missing HP and heal effective", () => {
    ...
    const attacker: any = makeAttacker({
